...
@property (strong, nonatomic) IBOutlet UIView *topImageContainerView;
...
//    [self searchImage : term];
    ...
    //
    [_topImageContainerView.layer setMasksToBounds:YES];
}
...
- (BOOL) isBadWord : (NSString *) word{
    for (NSString *badWord in APP_DELEGATE.badWordArray) {
        NSString *badWordLower = [badWord lowercaseString];
        word = [word lowercaseString];
        if ([word rangeOfString:badWordLower options:NSLiteralSearch].location != NSNotFound) {
            return YES;
        }
    }
    return NO;
